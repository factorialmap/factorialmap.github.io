...
![Aula convencional por volta de 1400 AD - Adaptado de Sarma- MIT 2020](aula_convencional.png){width="600" height="350"}
...
Foi exatamente isso que o pesquisador Benjamin @bloom1984 descobriu. Alunos que recebem ensino individualizado com tutoria têm um desempenho muito melhor, cerca de 2 desvios padrão acima dos alunos do ensino convencional. Isso é o que ele chamou de efeito 2 sigma.
...
# Dados para o aprendizado convencional (média mais baixa e maior dispersão)
convencional <- data.frame(
  tipo_aprendizado = "Convencional",
  pontuacao = rnorm(100, mean = 60, sd = 10) # Média de 60, desvio padrão de 10
)

# Dados para o aprendizado em aprendizagem_por_dominio (média um pouco maior, menor dispersão)
aprendizagem_por_dominio <- data.frame(
  tipo_aprendizado = "Aprendizagem por Domínio",
  pontuacao = rnorm(100, mean = 70.5, sd = 8) # Média de 70, desvio padrão de 8
)

# Dados para o aprendizado com tutoria_individual (média muito maior, menor dispersão ainda)
tutoria_individual <- data.frame(
  tipo_aprendizado = "Tutoria Individual",
  pontuacao = rnorm(100, mean = 74.9, sd = 5) # Média de 90, desvio padrão de 5
)

# Juntar todos os dados em um único dataframe
dados_aprendizado <- 
  bind_rows(convencional, aprendizagem_por_dominio, tutoria_individual) %>% 
  mutate(tipo_aprendizado = factor(tipo_aprendizado, levels = c("Convencional","Aprendizagem por Domínio","Tutoria Individual")))
...
dados_aprendizado %>% 
  ggdensity(x = "pontuacao",
            ...
            add = "mean",
            rug = TRUE,
            title = "Exemplo hipotérico de diferença nas notas de alunos",
            subtitle = "Linhas pontilhas são as médias",
            alpha = 0.3,
            ylab = "",
            legend.title = "Condição de aprendizagem",
...
resumo_aprendizado %>% 
  ...
  gt::tab_header(title = md("**Média e desvio padrão das notas**")) %>% 
  gt::cols_label(
    tipo_aprendizado = md("**Tipo aprendizado**"),
    media = md("**Média**"),
    desvio_padrao = md("**Desvio padrão**")
  )
```

```{r}
#| label: tabela_comparacao
#| echo: false
#| message: false
#| warning: false
#| fig-align: left
...
dados_aprendizado %>% 
  rstatix::cohens_d(pontuacao~tipo_aprendizado,
                    var.equal = TRUE,
                    ref.group = "Convencional") %>% 
  gt::gt() %>% 
  gt::fmt_number(decimals = 1) %>% 
  gt::tab_header(title = md("**Tamanho do efeito entre os grupos**")) %>% 
  gt::cols_label(
    .y. = md("Item avaliado"),
    group1 = md("Grupo Controle"),
    group2 = md("Grupo Tratamento"),
    effsize = md("Tamanho do efeito"),
    n1 = md("n grupo1"),
    n2 = md("n grupo2"),
...
1.  **Melhorar os pré-requisitos cognitivos dos alunos antes do início do curso**. Os detalhes sobre os pré-requisitos estão mais abaixo.
...
![Fluxo da abordagem de Layton](abordagem_layton.png)
...
data_edu_layton %>% 
  ...
  theme_minimal()+
  coord_flip()+
  scale_fill_manual(values = c("#1abc9c","#DE3163"))
